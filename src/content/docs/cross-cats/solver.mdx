...
Unlike polling, orders delivered over the websocket connected are not filterable. Any and all orders will be pushed to consumers. It is assumed that the consumer will filter orders locally.
...
Some orders may be solver exclusive. If `GetOrderData.order.orderData.verificationContract` is defined and not `address(0)`, it will be exclusive until `GetOrderData.order.orderData.slopeStartingTime` after which it is available for anyone.

If you hear 2 orders with the same `GetOrderData.order.swapper` and `GetOrderData.order.nonce` these are mutually exclusive. Only 1 of these can ever be submitted on-chain.

### Initiate Orders

After fetching and validating an order, it has to be submitted on-chain. Catalyst Orders arrive signed: `GetOrderData.signature`. The signature is dual-purpose. It is a permit2 signature that allows our contracts to pull the submittor's tokens and ensures that the user approves of the order. Orders will be initiated on a first come first serve basis.
...
Asset delivery depends on whether the destination is a VM chain or Bitcoin. For VM chains, you have to call the specified oracle with the appropriate configurations where for Bitcoin you just have to make any transfer that matches the provided arguments.
...
- The 31'th byte is the number of confirmations before the order can be verified on-chain. `0x00` and `0x01` is 1, `0x02` is 2, and `0x03` is 3...
- The 32'th byte contains an address version identifier. Decode as uint8.

If the transaction is to Bitcoin, the address (`GetOrderData.order.orderData.outputs[].recipient`) will contain the relevant destination hash or witness which is not the address! However, it has to be used to decode the address along with the address version identifier.
...
Nested witness addresses/outputs are **P2SH** and can be treated as any other **P2SH** address.

Once the address is generated, make a transaction that has 1 output that matches the **exactly** the described output from the initiated order. There is no limit as to how the transaction is organised, it may have any number inputs and outputs as long as one output matched exactly the one described by the order's output. This allows for transaction flexibility like batch filling, consolidation, and more. It may have more than one output that matches but this is not beneficial for anyone except the user.
...
While it is technically possible to create unique conditional deposits addresses, no wallet support pulling from custom scripts.
...
The Bitcoin address is encoded in 2 fields: `token` and `address`. The token field is used to block VM tokens from filling Bitcoin orders and to encode the address version. See this [table](#bitcoin-deliveries) for converting address types to versions.

The `address` is used to encode either the public key hash, script hash, or witness hash. The table [mentioned above](#bitcoin-deliveries) contains the encoding schemes for various Bitcoin address versions. Hashes of 20 bytes (P2PKH, P2SH, and P2WPKH) should be padded with 0s on the right side. (`0xabcdef...00000`)

### Quote Open Order (WIP)

Solvers for BTC to VM needs to be able to quote orders for comparison with other solvers. To start quoting, solvers needs to subscribe to requests quote from the order server. Upon requests for quotes, the solver needs to respond with a quote. The quote needs to be valid for at least 60 seconds: If within 30 seconds a signed order is requested, the solver needs to respond with a signed order that has a lifetime of 30 seconds.
